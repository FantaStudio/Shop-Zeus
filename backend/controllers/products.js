...
class products {
    async fetchProductsByAdmin(req, res) {
        try {
            ...
            return res.status(400).json({
                ...
                description:
                    "При запросе всех продуктов сайта возникла проблема",
            });
        }
    }
    ...
    async createProduct(req, res) {
        try {
            ...
            const imageHref = `${process.env.API_URL}/static/images/${file?.filename}`;
    ...
    async replaceImage(req, res) {
        try {
            ...
            product.imageHref = `${process.env.API_URL}/static/images/${file?.filename}`;
    ...
    async changeProduct(req, res) {
        try {
            ...
            product.countSimCards =
                body?.countSimCards ?? product.countSimCards;
            product.displayInInch =
                body?.displayInInch ?? product.displayInInch;
            ...
            product.supportGoogleMobileService =
                body?.supportGoogleMobileService ??
                product.supportGoogleMobileService;
            ...
            product.frontCameraMp =
                body?.frontCameraMp ?? product.frontCameraMp;
            ...
            product.cableInterface =
                body?.cableInterface ?? product.cableInterface;
            ...
            return res.status(400).json({
                message: "Возникла ошибка",
                description:
                    "При редактировании этого продукта возникла ошибка",
            });
        }
    }
